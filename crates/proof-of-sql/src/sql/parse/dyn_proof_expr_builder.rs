...
use proof_of_sql_parser::{
    intermediate_ast::{AggregationOperator, Literal},
...
impl DynProofExprBuilder<'_> {
    fn visit_expr(&self, expr: &Expr) -> Result<DynProofExpr, ConversionError> {
        match expr {
            Expr::Identifier(ident) => self.visit_column(ident.clone()),
            ...
            Expr::Function(func) => self.visit_function(func),
            Expr::AggregateExpressionWithFilter { expr, filter } => {
                if let Expr::Function(Function { name, .. }) = &**expr {
                    let Ok(op) = name.0[0].value.parse::<AggregationOperator>() {
                        return self.visit_aggregate_expr(op, filter);
                    }
                }
                Err(ConversionError::InvalidExpression {
                    expression: "Expected aggregation function in AggregateExpressionWithFilter"
                        .to_string(),
                })
            }
    ...
    fn visit_function(
        ...
        Err(ConversionError::Unprovable {
            error: format!("Function {func:?} is not supported yet"),
        })
    }
